...
from rest_framework.decorators import action, permission_classes  # 给自定义方法生成路由信息的装饰器
...
class ArticleModelViewSets(CacheResponseMixin, ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        article = Article.objects.create(author_id=request.data.pop('author'), category_id=request.data.pop("category"),
                                         **request.data)
    ...
    def ChangeOriginal(self, request, pk):
        ...
        # 创建序列化器
        serializer = self.get_serializer(instance=article, data={"original": not article.original},
